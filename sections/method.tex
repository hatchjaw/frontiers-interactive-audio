...
these are backed up by enumerations inspired by those used by
JackTrip.\footnote{
...
\texttt{BitResolution} could be used to transmit one of 8, 16, 24, or 32 as-is;
there is a utility, however, when decoding a packet, in knowing the number of
\textit{bytes} per audio sample, so this is the number that is represented, e.g.
\texttt{BitResolutionT::BIT16} takes the value 2, the number of bytes in a
16-bit integer.
...
\begin{figure}[ht]
    ...
    The audio processor receives audio channels from a multichannel source
        ...
        Samples are converted to the appropriate bit resolution
        % and endianness?
        and bundled into a UDP packet which is then written to the network.}
...
first byte that increases with each sample, that samples are transmitted
little-endian.
