...
Audio spatialisation is the practice of distributing sound in space.
In terms of the reproduction of primary sound sources, i.e.\ sound captured by
microphones, recorded as digital audio files, or synthesised in real-time,
spatialisation can be achieved simply by delivering those primary sources to
secondary sound sources, i.e.\ loudspeakers, placed at arbitrary positions
...
\textit{re}create) sonic environments for creative and immersive purposes, such
...
synthesis}\cite{ahrens_analytic_2012,nicol_sound_2017}, all of which
...
environment\cite{ahrens_analytic_2012}.
...
\end{equation}
where $f$ is time frequency and $\omega$ denotes radian frequency,
$\omega = 2\pi f$.
...
curtain'\cite{ziemer_wave_2020} entailed placing microphones in one space,
...
localisation\cite{ahrens_analytic_2012}.
...
components of increasing order\cite{nicol_sound_2017}.
...
exacerbates with attempts to reproduce sounds of higher frequency. The ideal
listening area can be broadened by implementing ambisonics at higher order,
increasing the density of the distribution of secondary sources.
...
WFS, reminiscent of the acoustic curtain, is based upon Huygens' principle,
...
sources\cite{mueller_acoustic_1971,berkhout_acoustic_1993,belloch_performance_2021}
(see \figref{fig:wfs_1}).
It is variously termed a form of \textit{acoustic holography} or
\textit{holophony}\cite{berkhout_holographic_1988,ahrens_analytic_2012}.
...
cannot be recreated unambiguously\cite{winter_geometric_2018},
...
% Methods include amplitude-based approaches such as vector based amplitude
% panning (VBAP)\cite{pulkki_virtual_1997} ... these approaches are
% \textit{periphonic}, creating phantom sound sources that emanate from the
% circumference of the listening area.

% \textit{Sound field synthesis}
% methods\cite{ahrens_analytic_2012,ziemer_wave_2020,nicol_sound_2017} are
% designed, by contrast, with the intent to physically (re)create a sound field.

% \dots In the frequency domain, the driving signal $\hat{D}$ for the secondary
% sound source at position
% $\mathbf{x}_0 = \begin{bmatrix} x_0 & y_0 & z_0 \end{bmatrix}^T$ is found as
% the product of an input signal $\hat{S}_{\text{in}}$ and a
% \textit{driving function} $D$\cite[s. 1.3]{ahrens_analytic_2012}:
% \begin{equation}\label{eq:driving-signal-freq}
%     \hat{D}(\bfx,\omega) = \Sigin(\omega) \cdot D(\bfx,\omega).
% \end{equation}
% For an amplitude panning scheme, $D(\bfx,\omega)$ reduces to a scalar
% constant value, or a time-varying function with scalar output, representing an
% appropriate weight to be applied to $\Sigin$ with respect
% to the secondary source at $\bfx$.
